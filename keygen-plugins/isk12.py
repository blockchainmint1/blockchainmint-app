"""
Iskander Coin Isk12 coin service — 12-word seed phrase Cold Storage Coin (ISK).

Drop this file into the `keygen` package used by csc-manager-ui
(https://github.com/blockchainmint1/csc-manager-ui) as:

    keygen/currencies/isk12_crypto_coin_service.py
...
exactly ONE chain. Isk12 mints the Iskander Coin edition:

  * The engraved secret (under the hologram/sticker) is the 12-word mnemonic.
  * The sticker printed on top carries the ISK public address + 6-char Asset ID
...
------------------------------------------------------------------------------
    ...
    python isk12.py generate --count 10 --out ./out
...
def isk_address_from_pubkey(pubkey_compressed: bytes) -> str:
    return _b58check_encode(bytes([ISK_PUBKEY_VERSION]) + _hash160(pubkey_compressed))


def isk_wif_from_privkey(priv32: bytes, compressed: bool = True) -> str:
    payload = bytes([ISK_SECRET_VERSION]) + priv32 + (b"\x01" if compressed else b"")
...
def _derive_node(seed_bytes, coin_type: int):
    ...
    Bip44Coins.BITCOIN hardcodes coin type 0, so ISK (SLIP-44 969696) has to
...
class Isk12CoinService(CoinService):
    """Iskander Coin Cold Storage Coin whose engraved secret is 12 seed words."""
    ...
    def __init__(self, coin_type: int = ISK_DEFAULT_COIN_TYPE):
    ...
    def get_coin_from_mnemonic(self, mnemonic: str) -> CryptoCoin:
        ...
        if len(words) != WORDS_COUNT:
            raise ValueError(
                "Isk12 requires a {}-word seed phrase (got {}).".format(
        ...
        coin = CryptoCoin(isk_address_from_pubkey(pub), mnemonic, mnemonic)
        coin.private_key = isk_wif_from_privkey(priv32)
        return coin
...
def _cmd_generate(args) -> int:
    service = Isk12CoinService(args.coin_type)
    ...
    print("Wrote {} ISK Isk12 coins to {}".format(len(coins), args.out))
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Iskander Coin Isk12 (12-word) CSC keygen")
    parser.add_argument(
        "--coin-type", type=int, default=ISK_DEFAULT_COIN_TYPE,
        help="SLIP-44 coin type used in the derivation path (default 969696)",
    )
    ...
    gen.add_argument("--coin-type", type=int, default=ISK_DEFAULT_COIN_TYPE)
    ...
    ver.add_argument("--coin-type", type=int, default=ISK_DEFAULT_COIN_TYPE)
