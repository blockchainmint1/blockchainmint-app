"""
Monero Xmr12 coin service — 12-word seed phrase Cold Storage Coin (BTC).

The engraved secret under the sticker is a 12-word BIP-39 mnemonic; the
sticker carries the XMR legacy address + 6-char Asset ID derived from it.

Monero mainnet: PUBKEY_ADDRESS=0 ("1…"), SECRET_KEY=128 (0x80), SLIP-44 coin type 0.
Keys derive at m/44'/0'/0'/0/0 (standard BIP-44), so the phrase restores
in any mainstream wallet.

Standalone use
--------------
    python xmr12.py generate --count 10 --out ./out
    python xmr12.py verify --seed "word ... word"
    python xmr12.py verify --seed "word ... word" --sticker "<address>,ABC123"
...
# Monero mainnet version bytes.
XMR_DEFAULT_COIN_TYPE = 128   # SLIP-44 coin type for Monero
...
def _monero_from_bip39_seed(seed_bytes: bytes):
    """Standard "Monero from a BIP-39 phrase" scheme.

    The BIP-39 seed is derived on the ed25519 SLIP-10 Monero path
    (m/44'/128'/0'/0/0); that private key becomes the Monero private spend
    key, from which the view key and the primary address follow. This is the
    same scheme used by Ledger and by bip_utils' Monero support, so the phrase
    restores in any wallet that offers "Monero from BIP-39 seed".
    """
    try:
        from bip_utils import Bip44, Bip44Coins, Monero
    except ImportError as exc:  # pragma: no cover - bip_utils 1.7.0
        raise RuntimeError(
            "Monero support needs bip_utils >= 2.x (pip install -U bip_utils)."
        ) from exc
    node = Bip44.FromSeed(seed_bytes, Bip44Coins.MONERO_ED25519_SLIP).DeriveDefaultPath()
    return Monero.FromBip44PrivateKey(node.PrivateKey().Raw().ToBytes())
...
def _derive_node(seed_bytes, coin_type: int):
    ...
    Bip44Coins.BITCOIN hardcodes coin type 0, so XMR (SLIP-44 696969) has to
...
class Xmr12CoinService(CoinService):
    """Monero Cold Storage Coin whose engraved secret is 12 seed words."""
    ...
    def __init__(self, coin_type: int = XMR_DEFAULT_COIN_TYPE):
    ...
    def get_coin_from_mnemonic(self, mnemonic: str) -> CryptoCoin:
        ...
        if len(words) != WORDS_COUNT:
            raise ValueError(
                "Xmr12 requires a {}-word seed phrase (got {}).".format(
        ...
        monero = _monero_from_bip39_seed(seed_bytes)
        address = monero.PrimaryAddress()
        # csc-manager writes coin.wif into key.txt (the engraved secret), so the
        # mnemonic goes there. The spend key lives on .private_key.
        coin = CryptoCoin(address, mnemonic, mnemonic)
        coin.private_key = monero.PrivateSpendKey().Raw().ToHex()
        coin.view_key = monero.PrivateViewKey().Raw().ToHex()
        return coin
    ...
    def get_csv_header(self):
        return "seed,address,spend_key,view_key,derivation\n"

    def derivation_path(self) -> str:
        return "m/44'/{}'/0'/0/0 (ed25519 SLIP-10 -> Monero spend key)".format(
            self.coin_type
        )
    ...
    def format(self, coin: CryptoCoin) -> str:
        return '"{}",{},{},{},{}\n'.format(
            ...
            getattr(coin, "view_key", ""), self.derivation_path(),
        )
...
def _cmd_generate(args) -> int:
    service = Xmr12CoinService(args.coin_type)
    ...
    _write(j("viewkeys.txt"), ["{}\n".format(getattr(c, "view_key", "")) for c in coins])
    ...
    print("Wrote {} XMR Xmr12 coins to {}".format(len(coins), args.out))
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Monero Xmr12 (12-word) CSC keygen")
    parser.add_argument(
        "--coin-type", type=int, default=XMR_DEFAULT_COIN_TYPE,
    ...
    gen.add_argument("--coin-type", type=int, default=XMR_DEFAULT_COIN_TYPE)
    ...
    ver.add_argument("--coin-type", type=int, default=XMR_DEFAULT_COIN_TYPE)
