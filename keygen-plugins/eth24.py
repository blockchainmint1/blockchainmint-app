"""
Ethereum / EVM Eth24 coin service — 24-word seed phrase Cold Storage Coin.

Drop this file into the `keygen` package used by csc-manager-ui
(https://github.com/blockchainmint1/csc-manager-ui) as:

    keygen/currencies/eth24_crypto_coin_service.py
...
exactly ONE chain. Eth24 mints the EVM edition:

  * The engraved secret (under the hologram/sticker) is the 24-word mnemonic.
...
------------------------------------------------------------------------------
    ...
    python eth24.py generate --count 10 --out ./out
    ...
    python eth24.py verify --seed "word ... word" --sticker "0xabc...,ABC123"
...
WORDS_COUNT = 24
ENTROPY_BYTES = 32  # 256 bits — BIP-39 strength for 24 words
...
class Eth24CoinService(CoinService):
    ...
    def get_coin_from_mnemonic(self, mnemonic: str) -> CryptoCoin:
        ...
        if len(words) != WORDS_COUNT:
            raise ValueError(
                "Eth24 requires a {}-word seed phrase (got {}).".format(
...
def _cmd_generate(args) -> int:
    service = Eth24CoinService(args.coin_type)
    ...
    print("Wrote {} ETH Eth24 coins to {}".format(len(coins), args.out))
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ethereum/EVM Eth24 (24-word) CSC keygen")
    parser.add_argument(
    ...
    ver.add_argument("--seed", required=True, help='24 words, or "-" to read stdin')
