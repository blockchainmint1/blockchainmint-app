"""
TEXITcoin Txc24 coin service — 24-word seed phrase Cold Storage Coin (TXC).

Drop this file into the `keygen` package used by csc-manager-ui
(https://github.com/blockchainmint1/csc-manager-ui) as:

    keygen/currencies/txc24_crypto_coin_service.py
...
exactly ONE chain. Txc24 mints the TEXITcoin edition:
...
------------------------------------------------------------------------------
    ...
    python txc24.py generate --count 10 --out ./out
    ...
    python txc24.py verify --seed "word ... word" --sticker "Txxxxxxx,ABC123"
...
class Txc24CoinService(CoinService):
    ...
    def get_coin_from_mnemonic(self, mnemonic: str) -> CryptoCoin:
        ...
        if len(words) != WORDS_COUNT:
            raise ValueError(
                "Txc24 requires a {}-word seed phrase (got {}).".format(
...
def _cmd_generate(args) -> int:
    service = Txc24CoinService(args.coin_type)
    ...
    print("Wrote {} TXC Txc24 coins to {}".format(len(coins), args.out))
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(description="TEXITcoin Txc24 (24-word) CSC keygen")
    parser.add_argument(
