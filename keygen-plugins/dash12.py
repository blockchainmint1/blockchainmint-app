"""
Dash Dash12 coin service — 12-word seed phrase Cold Storage Coin (DASH).

The engraved secret under the sticker is a 12-word BIP-39 mnemonic; the
sticker carries the DASH legacy address + 6-char Asset ID derived from it.

Dash mainnet: PUBKEY_ADDRESS=76 ("X…"), SECRET_KEY=204 (0xCC), SLIP-44 coin type 5.
Keys derive at m/44'/5'/0'/0/0 (standard BIP-44), so the phrase restores
in any mainstream wallet.

Standalone use
--------------
    python dash12.py generate --count 10 --out ./out
    python dash12.py verify --seed "word ... word"
    python dash12.py verify --seed "word ... word" --sticker "<address>,ABC123"
...
DASH_PUBKEY_VERSION = 0x4C
DASH_SECRET_VERSION = 0xCC
DASH_DEFAULT_COIN_TYPE = 5
...
def dash_address_from_pubkey(pubkey_compressed: bytes) -> str:
    return _b58check_encode(bytes([DASH_PUBKEY_VERSION]) + _hash160(pubkey_compressed))


def dash_wif_from_privkey(priv32: bytes, compressed: bool = True) -> str:
    payload = bytes([DASH_SECRET_VERSION]) + priv32 + (b"\x01" if compressed else b"")
...
def _derive_node(seed_bytes, coin_type: int):
    ...
    Bip44Coins.BITCOIN hardcodes coin type 0, so DASH (SLIP-44 696969) has to
...
class Dash12CoinService(CoinService):
    """Dash Cold Storage Coin whose engraved secret is 12 seed words."""
    ...
    def __init__(self, coin_type: int = DASH_DEFAULT_COIN_TYPE):
    ...
    def get_coin_from_mnemonic(self, mnemonic: str) -> CryptoCoin:
        ...
        if len(words) != WORDS_COUNT:
            raise ValueError(
                "Dash12 requires a {}-word seed phrase (got {}).".format(
        ...
        coin = CryptoCoin(dash_address_from_pubkey(pub), mnemonic, mnemonic)
        coin.private_key = dash_wif_from_privkey(priv32)
        return coin
...
def _cmd_generate(args) -> int:
    service = Dash12CoinService(args.coin_type)
    ...
    print("Wrote {} DASH Dash12 coins to {}".format(len(coins), args.out))
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Dash Dash12 (12-word) CSC keygen")
    parser.add_argument(
        "--coin-type", type=int, default=DASH_DEFAULT_COIN_TYPE,
        help="SLIP-44 coin type used in the derivation path (default 5)",
    )
    ...
    gen.add_argument("--coin-type", type=int, default=DASH_DEFAULT_COIN_TYPE)
    ...
    ver.add_argument("--coin-type", type=int, default=DASH_DEFAULT_COIN_TYPE)
