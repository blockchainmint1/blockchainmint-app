"""
Litecoin Ltc12 coin service — 12-word seed phrase Cold Storage Coin (LTC).

The engraved secret under the sticker is a 12-word BIP-39 mnemonic; the
sticker carries the LTC legacy address + 6-char Asset ID derived from it.

Litecoin mainnet: PUBKEY_ADDRESS=48 ("L…"), SECRET_KEY=176 (0xB0), SLIP-44 coin type 2.
Keys derive at m/44'/2'/0'/0/0 (standard BIP-44), so the phrase restores
in any mainstream wallet.

Standalone use
--------------
    python ltc12.py generate --count 10 --out ./out
    python ltc12.py verify --seed "word ... word"
    python ltc12.py verify --seed "word ... word" --sticker "<address>,ABC123"
...
LTC_PUBKEY_VERSION = 0x30
LTC_SECRET_VERSION = 0xB0
LTC_DEFAULT_COIN_TYPE = 2
...
def ltc_address_from_pubkey(pubkey_compressed: bytes) -> str:
    return _b58check_encode(bytes([LTC_PUBKEY_VERSION]) + _hash160(pubkey_compressed))


def ltc_wif_from_privkey(priv32: bytes, compressed: bool = True) -> str:
    payload = bytes([LTC_SECRET_VERSION]) + priv32 + (b"\x01" if compressed else b"")
...
def _derive_node(seed_bytes, coin_type: int):
    ...
    Bip44Coins.BITCOIN hardcodes coin type 0, so LTC (SLIP-44 696969) has to
...
class Ltc12CoinService(CoinService):
    """Litecoin Cold Storage Coin whose engraved secret is 12 seed words."""
    ...
    def __init__(self, coin_type: int = LTC_DEFAULT_COIN_TYPE):
    ...
    def get_coin_from_mnemonic(self, mnemonic: str) -> CryptoCoin:
        ...
        if len(words) != WORDS_COUNT:
            raise ValueError(
                "Ltc12 requires a {}-word seed phrase (got {}).".format(
        ...
        coin = CryptoCoin(ltc_address_from_pubkey(pub), mnemonic, mnemonic)
        coin.private_key = ltc_wif_from_privkey(priv32)
        return coin
...
def _cmd_generate(args) -> int:
    service = Ltc12CoinService(args.coin_type)
    ...
    print("Wrote {} LTC Ltc12 coins to {}".format(len(coins), args.out))
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Litecoin Ltc12 (12-word) CSC keygen")
    parser.add_argument(
        "--coin-type", type=int, default=LTC_DEFAULT_COIN_TYPE,
        help="SLIP-44 coin type used in the derivation path (default 2)",
    )
    ...
    gen.add_argument("--coin-type", type=int, default=LTC_DEFAULT_COIN_TYPE)
    ...
    ver.add_argument("--coin-type", type=int, default=LTC_DEFAULT_COIN_TYPE)
