...
engraves ONLY a 12-word BIP-39 mnemonic (128 bits of entropy). So:
...
------------------------------------------------------------------------------
    python bee12.py --count 10 --chain BTC --out ./out
...
#
# `Bip39WordsNum.WORDS_NUM_12` -> 128-bit entropy -> 12 words, for every chain.
# --------------------------------------------------------------------------
BEEKEEPER_CHAINS = {
...
class Bee12CoinService(CoinService):
    ...
    def get_coin(self, private_key: str) -> CryptoCoin:
        """`private_key` for Beekeeper is the 12-word mnemonic itself."""
    ...
    def get_coin_from_mnemonic(self, mnemonic: str) -> CryptoCoin:
        ...
        if len(words) != WORDS_COUNT:
            raise ValueError(
                "Beekeeper Bee12 requires a {}-word seed phrase (got {}).".format(
...
#
# Beekeeper Bee12 writes the 12-word mnemonic into key.txt because that IS the
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Beekeeper Bee12 (12-word) CSC keygen")
    ...
    service = Bee12CoinService(args.chain)
    ...
    print("Wrote {} Beekeeper Bee12 {} coins to {}".format(len(coins), args.chain, args.out))
